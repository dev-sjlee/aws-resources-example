$CLUSTER_NAME = $args[0]
$SERVICE_ACCOUNT_NAMESPACE = $args[1]
$REGION = $args[2]
$PROJECT_NAME = $args[3]
$AWS_ACCOUNT_ID = aws sts get-caller-identity --query "Account" --output text
$OIDC_PROVIDER = aws eks describe-cluster --name $CLUSTER_NAME --query "cluster.identity.oidc.issuer" --output text | sed -e "s/^https:\/\///" --region $REGION
...
"@ | Tee-Object -FilePath "TrustPolicy.json" -Append
#
...
"@ | Tee-Object -FilePath "PermissionPolicyIngest.json" -Append

function getRoleArn($roleName) {
  $OUTPUT = aws iam get-role --role-name $roleName --query 'Role.Arn' --region $REGION --output text 2>&1

  # Check for an expected exception
  if ( $LASTEXITCODE -eq 0 ) {
    Write-Output $OUTPUT
  } elseif ( $OUTPUT -like '*NoSuchEntity*' ) {
    Write-Output ""
  } else {
    Write-Error $OUTPUT
    return 1
